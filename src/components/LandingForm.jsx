...
const LandingForm = ({ show, handleClose }) => {
...
const validate = () => {
  ...
  if (!formData.isDentist) newErrors.isDentist = "Please select Yes or No";
  if (!formData.hasClinic) newErrors.hasClinic = "Please select Yes or No";
  if (!formData.usesSoftware) newErrors.usesSoftware = "Please select Yes or No";

   // ✅ Conditional check
  if (formData.hasClinic === "yes" && !formData.clinic) {
    newErrors.clinic = "Clinic Name is required if you have a clinic";
  }
...
const handleChange = (e) => {
  ...
  // If user changes the state, update city options and reset city
  if (name === 'state') {
    setAvailableCities(cityData[value] || []);
    setFormData((prev) => ({
      ...prev,
      state: value,
  ...
  setErrors((prev) => ({ ...prev, [name]: '' }));
   ...
   const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://erp4dentist.com/newlanding.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
          usname: formData.name,
          usmobile: formData.phone,
          usemail: formData.email,
          usstate: formData.state,
          uscity: formData.city,
           // ✅ Send all checkbox/radio data
          isDentist: formData.isDentist,
          hasClinic: formData.hasClinic,
          usesSoftware: formData.usesSoftware,
          // ✅ Only send clinic data if hasClinic is "yes"
          ...(formData.hasClinic === "yes" && { clinic: formData.clinic })
        }).toString(),
      });
  ...
  return (
    <Modal show={show} onHide={handleClose} centered>
      <Modal.Header closeButton>
        <Modal.Title>Landing Form</Modal.Title>
      </Modal.Header>
      <Modal.Body className="signin signup">
        <Form onSubmit={handleSubmit}>
          ...
          {/* <Form.Group className="mb-3">
            ...
            />
            <Form.Control.Feedback type="invalid">{errors.clinic}</Form.Control.Feedback>
          </Form.Group> */}
       ...
       <Form.Group className="mb-3">
            <Form.Label>Phone Number</Form.Label>
                <input
                  ...
                  onChange={(e) => {
                    const cleanedValue = e.target.value.replace('+91', '');
...
<Form.Group className="mb-3">
  <Form.Label>Are you a dentist?*</Form.Label>
  <div>
    <Form.Check
      inline
      type="radio"
      ...
      name="isDentist"
      value="yes"
      checked={formData.isDentist === "yes"}
      onChange={handleChange}
    />
    <Form.Check
      inline
      ...
      name="isDentist"
      value="no"
      checked={formData.isDentist === "no"}
      onChange={handleChange}
    />
  </div>
  <Form.Control.Feedback type="invalid">{errors.isDentist}</Form.Control.Feedback>
</Form.Group>

<Form.Group className="mb-3">
  <Form.Label>Do you have a clinic?*</Form.Label>
  <div>
    <Form.Check
      inline
      type="radio"
      ...
      name="hasClinic"
      value="yes"
      checked={formData.hasClinic === "yes"}
      onChange={handleChange}
    />
    <Form.Check
      inline
      ...
      name="hasClinic"
      value="no"
      checked={formData.hasClinic === "no"}
      onChange={handleChange}
    />
  </div>
  <Form.Control.Feedback type="invalid">{errors.hasClinic}</Form.Control.Feedback>
</Form.Group>

  {/* ✅ Conditionally rendered Clinic Name field */}
          {formData.hasClinic === "yes" && (
            <Form.Group className="mb-3">
              <Form.Label>Clinic Name*</Form.Label>
              <Form.Control
                type="text"
...
<Form.Group className="mb-3">
  <Form.Label>Do you use any dental software?*</Form.Label>
  <div>
    <Form.Check
      inline
      type="radio"
      ...
      name="usesSoftware"
      value="yes"
      checked={formData.usesSoftware === "yes"}
      onChange={handleChange}
    />
    <Form.Check
      inline
      ...
      name="usesSoftware"
      value="no"
      checked={formData.usesSoftware === "no"}
      onChange={handleChange}
    />
  </div>
  <Form.Control.Feedback type="invalid">{errors.usesSoftware}</Form.Control.Feedback>
</Form.Group>
 ...
 <Form.Group className="mb-3">
  ...
  <Form.Control.Feedback type="invalid">{errors.city}</Form.Control.Feedback>
          ...
          <Button variant="sub" type="submit" disabled={loading} className="w-100">
